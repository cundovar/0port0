...
const ModalDetailpage = ({ onClose, project }) => {
  ...
  if (project) {
    if (project.imageSrc.length > 1) {
      modalContent = (
        <>
          <div className="absolute h-full flex items-center w-full max-md:visible lg:invisible backdrop-blur-xl z-50   ">
            <div className="  relative  lg:w-1/2 w-full h-1/2 ">
            <button
              ...
              <Slider {...settings} ref={sliderRef}>
                {project.imageSrc.map((image, index) => (
                  <div key={index} className=" bg-stone-900 overflow-hidden">
                    <img className=" h-52 m-auto" src={image} alt={project.titre} />
                  </div>
                      ...
                      <div className=" flex flex-col mt-5 w-full imageListe space-y-5  bg-opacity-50 bg-slate-700 p-1 m-4 items-start justify-start ">
                        <div className="w-full flex max-lg:flex-col  ">
                          <div className=" w-full  flex flex-col ">
                            <h2 className=" lg:text-xl text-lg mb-5">{project.titre}</h2>
                          </div>
